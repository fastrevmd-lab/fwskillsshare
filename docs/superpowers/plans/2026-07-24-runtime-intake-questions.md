...
- `sis_policy_model`; header `Policy model`; ask when the baseline policy
  ...
  to-zone fields; `Zone-pair by exception` — Route the policy stage for zone-pair design under a named exception documented in baseline-policy.md (srx-policy, which covers Branch and non-Branch platforms); the caller states which exception applies and why.
