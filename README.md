...
<p align="center">
  <img alt="skills" src="https://img.shields.io/badge/skills-30-0D9488">
  <img alt="reviewed" src="https://img.shields.io/badge/reviewed-26%2F30-262B38">
...
**30 skills** across five families. All of them are **model-invoked** — the agent reaches for them automatically when it sees vendor keywords, an SRX operational topic, a Security Director On-Prem or ClearPass deployment request, or compliance language in your message or a pasted config. 26 of the 30 packages have completed the review record below; `clearpass-proxmox-deploy`, `parsing-firepower-configs`, `srx-syslog-logging`, and `srx-idp` are the exceptions — the first is a draft, the last is a draft pending vSRX validation, and the middle two have not yet been reviewed. Invoke one explicitly as `/srx-nat` in Claude Code or Hermes, or `$srx-nat` in Codex.
...
are `clearpass-proxmox-deploy`, which ships as a draft, the draft `srx-idp`, and
`parsing-firepower-configs` and `srx-syslog-logging`, which have not yet been through the two-stage review. Four review rounds, the
