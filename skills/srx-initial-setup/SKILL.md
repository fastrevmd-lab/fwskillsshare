...
3. **Policy design beyond the baseline.** This skill establishes the minimum security policy required to make the device usable: outbound DNS, web, and NTP, plus default-deny with logging. The baseline is generated using global policies by default; a documented zone-pair opt-out exists for specific exceptions (see `references/stages/baseline-policy.md`). Application-aware policies, URL filtering, intrusion prevention, and advanced security services route to `srx-policy`.
