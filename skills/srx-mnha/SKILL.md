...
Grid model, field-confirmed on **vSRX 26.2R1.7** (routed pair, SRG1
`deployment-type routing`). Node A — mirror on Node B with swapped IDs (`local-id 2`,
`local-domain-id 2`, `peer-domain-id 1 peer-id 1`, swapped ICL IPs, lower
`activeness-priority`):
...
Additional behaviors confirmed on **vSRX 26.2R1.7** — two routed MNHA pairs (core +
edge), grid model, SRG1 `deployment-type routing`, eBGP + signal-route MED steering
(2026-07):

- **Grid model required on 26.x** (see "Config model" section). The flat form
  commits but never activates until you switch to `grid-id`/`local-domain-id`/
  `peer-domain-id` **and reboot**.
- **SRG1 election may not converge → active/active.** ICL cold sync stuck at
  ...
  signal route and advertised the better MED → neighbors ECMP across both nodes and
  stateful cold-sync never completed. **Routing failover still works** (kill a node,
  the survivor keeps the route); only *stateful* failover is lost. Verify via
  `Conn State`/`Cold Sync Status`, not just `Node Status`.
- **Advertise connected transit subnets in the signal-route export**, not just
  learned routes — otherwise the peer can't route back to on-transit sources (SNAT
  address, a downstream firewall's transit IP) and return traffic black-holes. Add
  a `from protocol direct … route-filter <transit> exact accept` term (pitfall 21).
- **Active/active SNAT: give each node its own pool + proxy-ARP address** so they
  never both answer ARP for one IP; return traffic follows the pool that translated
  it, no VIP needed. Sessions don't survive a failover that changes the pool
  (acceptable where cold-sync isn't converging).
...
- **Virtualized SRX (vSRX on KVM/Proxmox):** a soft `request system reboot` can
  ...
  then `load set` (no scp subsystem); delete a fresh clone's `fxp0 family inet dhcp`
  before adding a static address or the commit fails the constraint check.
