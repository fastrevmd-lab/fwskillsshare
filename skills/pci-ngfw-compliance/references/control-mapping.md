...
| 8.2.5 Terminated users | Admin and VPN access immediately revoked on termination | Offboarding checklist, account-deletion evidence tied to HR separation dates |
...
| 8.5.1 MFA system protections | MFA implementation is replay-resistant, cannot be bypassed, requires at least two different factor types, and grants access only after all factors succeed (fatigue/push-bombing resistance is good practice, guidance-level) | MFA system config review; evidence of replay-resistance and anti-bypass controls |
| 5.2/5.3 Anti-malware support | NGFW anti-malware/IPS/DNS-security features can supplement (not replace) host anti-malware for systems at risk | AV/IPS profile config, signature update status, sample threat logs |
